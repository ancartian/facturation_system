...
def total():
    ...
    sub_totla =sub_total_comida + sub_total_bebida + sub_total_postres
    impuestos = sub_totla * 0.07
    total =sub_totla + impuestos

    var_costo_comida.set(f"${round(sub_total_comida,2)}")
    var_costo_bebida.set(f"${round(sub_total_bebida, 2)}")
    var_costo_postres.set(f"${round(sub_total_postres, 2)}")
    var_subtotal.set(f"${round(sub_totla,2)}")
    var_impuestos.set(f"$ {round(sub_total_comida,2)}")
...
aplicacion.geometry('1130x600+0+0')  # 1020x630+0+0'

# Evitar maximixar la ventana
...
etiqueta_titulo = Label(panel_superior, text='Sistema de Facturacion', fg='azure',
                        font=('Dosis', 48), bg='DodgerBlue', width=27)
...
# Panel Calculadora
panel_calculadora = Frame(panel_derecha, bd=1, relief=FLAT, bg='DodgerBlue')
panel_calculadora.pack()

# Panel recibo
panel_recibo = Frame(panel_derecha, bd=1, relief=FLAT, bg='DodgerBlue')
panel_recibo.pack()

# Panel botones
panel_botones = Frame(panel_derecha, bd=1, relief=FLAT, bg='DodgerBlue')
panel_botones.pack()
